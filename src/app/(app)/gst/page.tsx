...
export default function GstLeadsPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        setLoading(true);
        await Promise.all([fetchGstLeads(), fetchLawyers()]);
        setLoading(false);
    }
    fetchData();
  }, []);
  ...
  const downloadAsCSV = (data: Lead[]) => {
    ...
    a.setAttribute('download', 'gst_leads.csv');
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between gap-4">
          <div>
            <CardTitle>GST Leads</CardTitle>
            <CardDescription>
              Manage all your leads with the GST tag.
            </CardDescription>
          </div>
          <div className="flex items-center gap-2">
             <Input 
      ...
      <CardContent>
        <Table>
          ...
          <TableBody>
            {loading ? (
                <TableRow>
                    <TableCell colSpan={8} className="text-center h-24">Loading GST leads...</TableCell>
                </TableRow>
            ) : sortedAndFilteredLeads.length === 0 ? (
                 <TableRow>
                    <TableCell colSpan={8} className="text-center h-24">No GST leads found.</TableCell>
                </TableRow>
            ) : (
